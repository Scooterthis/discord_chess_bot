...
export class Pawn extends Piece {
     ...
     */
    constructor (tile: Tile, captured: boolean = false, isWhite: boolean = false, hasMoved: boolean = false, enPassant: boolean = false) {
    ...
    static fromPawn (pawn: Pawn): Pawn {
        return new this(
            Tile.fromTile(pawn.tile),
            pawn.captured,
            pawn.isWhite,
            pawn.hasMoved,
            pawn.enPassantEligable
        );
    }
    ...
    public generateMoves (currentPlayer: Player, enemyPlayer: Player, board: Board): Array<Tile> {
        ...
        if(currentPlayer.isWhite){
            ...
            if (enemyPlayer.piecePresent(new Tile(position.x - 1, position.y))) {
                let ePiece = enemyPlayer.getPieceByTile(new Tile(position.x - 1, position.y)); 
