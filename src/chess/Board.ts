...
export class Board {
     ...
     */
    public inBoard (tile: Tile): boolean {
     ...
     */
    public printBoard(white: Player, black: Player): string {
        let boardString = "";
        let pieces: Array<string> = new Array<string>();
        for(let i = 0; i < this.tiles.length; i++){
            if(white.getPieceByTile(this.tiles[i]) instanceof Queen && !white.getPieceByTile(this.tiles[i]).captured){
                pieces.push("wQ");
            }
            else if(white.getPieceByTile(this.tiles[i]) instanceof King && !white.getPieceByTile(this.tiles[i]).captured){
                pieces.push("wK");
            }
            else if(white.getPieceByTile(this.tiles[i]) instanceof Knight && !white.getPieceByTile(this.tiles[i]).captured){
                pieces.push("wk");
            }
            else if(white.getPieceByTile(this.tiles[i]) instanceof Rook && !white.getPieceByTile(this.tiles[i]).captured){
                pieces.push("wR");
            }
            else if(white.getPieceByTile(this.tiles[i]) instanceof Bishop && !white.getPieceByTile(this.tiles[i]).captured){
                pieces.push("wB");
            }
            else if(white.getPieceByTile(this.tiles[i]) instanceof Pawn && !white.getPieceByTile(this.tiles[i]).captured){
                pieces.push("wP");
            }
            else if(black.getPieceByTile(this.tiles[i]) instanceof Queen && !black.getPieceByTile(this.tiles[i]).captured){
                pieces.push("bQ");
            }
            else if(black.getPieceByTile(this.tiles[i]) instanceof King && !black.getPieceByTile(this.tiles[i]).captured){
                pieces.push("bK");
            }
            else if(black.getPieceByTile(this.tiles[i]) instanceof Knight && !black.getPieceByTile(this.tiles[i]).captured){
                pieces.push("bk");
            }
            else if(black.getPieceByTile(this.tiles[i]) instanceof Rook && !black.getPieceByTile(this.tiles[i]).captured){
                pieces.push("bR");
            }
            else if(black.getPieceByTile(this.tiles[i]) instanceof Bishop && !black.getPieceByTile(this.tiles[i]).captured ){
                pieces.push("bB");
            }
            else if(black.getPieceByTile(this.tiles[i]) instanceof Pawn && !black.getPieceByTile(this.tiles[i]).captured){
                pieces.push("bP");
            }
        else pieces.push("  ");
        }
